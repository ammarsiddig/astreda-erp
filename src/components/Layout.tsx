...
function getRoleBadgeColor(roleName: string): string {
  ...
  if (roleName === 'مدير') return 'bg-[#003D6B] text-white';
...
export default function Layout() {
  ...
  return (
    <div className="flex h-screen bg-[#F3F4F6] font-sans text-slate-900 overflow-hidden" dir={lang === 'ar' ? 'rtl' : 'ltr'}>

      {/* Sidebar — Desktop */}
      <aside className="hidden md:flex flex-col w-64 bg-[#003D6B] text-white shadow-xl z-20 flex-shrink-0">
        <SidebarContent />
      </aside>
        ...
        {isMobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className={cn(
                'fixed top-0 bottom-0 w-64 bg-[#003D6B] text-white shadow-2xl z-40 flex flex-col',
